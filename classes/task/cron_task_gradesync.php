...
class cron_task_gradesync extends \core\task\scheduled_task {
    ...
    */
    ...
    protected function cleanup_mappings() {
        ...
        $this->log("Starting mapping cleanup", 1);
        ...
        $courseids = array_values(array_column($DB->get_records_sql($sql), 'id'));
        // Determine inactive courses.
        $activemappings = array_intersect($courseids, $this->mappingcourses);
        $inactivemappings = array_diff($this->mappingcourses, $activemappings);
        ...
        // Get list of viable groups.
        list($insql, $inparams) = $DB->get_in_or_equal($courseids);
        $sql = "SELECT id
                  FROM {groups}
                 WHERE courseid $insql";
        $groupids = array_values(array_column($DB->get_records_sql($sql, $inparams), 'id'));
        ...
        $activemappings[] = 0; // Course level mappings have always default to 0 for groupid.
        $inactivemappings = array_diff($mappinggroups, $activemappings);
        // Delete inactive mappings.
        if ($inactivemappings) {
